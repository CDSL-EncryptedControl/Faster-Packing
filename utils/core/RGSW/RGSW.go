...
func EncFi(F [][]float64, v int, encryptorRGSW *rgsw.Encryptor, levelQ int, levelP int, ringQ *ring.Ring, params rlwe.Parameters) *rgsw.Ciphertext {
	n := len(F)
	Fbar := make([][]float64, n)
	...
	if v < 1 {
		Fbar[n-1][0] = Fbar[n-1][0] + 1
	} else { 
		Fbar[v-1][0] = Fbar[v-1][0] - 1
	}
